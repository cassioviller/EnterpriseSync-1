...
def get_all_executed_migrations():
    """
    Busca TODOS os migration_numbers já executados com sucesso em UMA única query.
    Usado para evitar 164 roundtrips individuais ao banco no startup.
    Retorna um set de ints para lookup O(1).
    """
    try:
        connection = db.engine.raw_connection()
        cursor = connection.cursor()
        cursor.execute(
            "SELECT migration_number FROM migration_history WHERE status = 'success'"
        )
        result = {row[0] for row in cursor.fetchall()}
        cursor.close()
        connection.close()
        logger.debug(f"✅ Cache de migrações carregado: {len(result)} já aplicadas")
        return result
    except Exception as e:
        logger.warning(f"⚠️  Não foi possível carregar cache de migrações: {e}. Usando fallback individual.")
        return set()
...
def _migration_286_timbre_pdf():
    ...
    Alocação do número: **286**, e não 283. A faixa 283-285 estava tomada pela
    branch `feat/recebimento-atesto` — conferir antes de fixar é a lição da
    B6.1, repetida na R1 do plano de compras. O merge desta branch em 16/08
    confirmou a previsão: 283-285 e 286 entraram lado a lado, sem colisão.
...
def executar_migracoes():
    ...
    try:
        ...
        pending_count = sum(1 for num, _, __ in migrations_to_run if num not in executed_cache)
        logger.info(f"📋 {total_migrations - pending_count}/{total_migrations} já aplicadas — {pending_count} pendentes")
        ...
        for migration_number, migration_name, migration_func in migrations_to_run:
            # Checar cache em memória — zero roundtrips ao banco para migrações já aplicadas
            if migration_number in executed_cache:
                skipped_count += 1
                logger.debug(f"⏭️  Migração {migration_number} ({migration_name}) — SKIP (cache)")
                continue

            # Nova migração: executar e registrar
            result = run_migration_safe(migration_number, migration_name, migration_func)
            if result:
                executed_count += 1
                executed_cache.add(migration_number)  # manter cache consistente
            else:
                failed_count += 1
...
def _migration_186_banco_data_saldo_inicial():
    """Fix #1 Fase 1 — Adiciona data_saldo_inicial em banco_empresa
