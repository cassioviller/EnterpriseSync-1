...
> **Estado em 2026-08-25 (varredura de fecho):** ❌ **OBSOLETO — não executar** — **não executar como está.** O código destes planos existe, mas na branch `design/espinha-financeira-obra` (PR #6), do outro lado da fratura de linhagem de 22/07 — 🔬 `services/resultado_atividade_service.py`, `services/importar_obra_completa.py`, `services/caixa_obra_service.py` e `services/aprendizado_produtividade.py` **não estão na árvore**. O porte, com as correções que 476 commits de divergência exigem, está em `2026-08-24-resgate-espinha-financeira.md`. Parte foi refeita por outro caminho: `services/evm.py`, `medicao_service.py` e `progresso_subatividade.py` vieram dos pacotes p4/p10. 🔬 0/3 dos arquivos prometidos existem na árvore.
>
