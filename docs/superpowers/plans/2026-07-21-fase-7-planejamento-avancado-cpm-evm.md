...
Reconferência do plano contra o código de `main` em 2026-07-23 (commit `8948573a` — Fases 1, 1.5, 2 e 3 mergeadas; migrations aplicadas até **247**). Cada linha foi verificada **abrindo o arquivo**, não deduzida pelo nome. Legenda: **CONFIRMADA** (vale como escrito, evidência atual) · **QUEBRADA** (o texto do plano ficou falso; ajuste indicado) · **NOVA** (risco que as Fases 1–3 criaram e o plano não previu).

### Premissas numeradas (seção 5)

| # | Premissa | Veredito | Evidência (23/07) | Ajuste necessário |
|---|---|---|---|---|
| P1 | Faixa 280–289 livre | **CONFIRMADA, com refs velhas** | Maior migration registrada: **247** (`migrations.py:4770`), não 213; registro `migrations_to_run` mudou de `:3831` para **`migrations.py:4567`**; zero ocorrências de 280–283 no arquivo. A Fase 4, em andamento no worktree `.claude/worktrees/fase-4`, usa 250–254 — sem colisão | Nenhum no código. **Atenção à reserva:** `ESTADO-ATUAL.md` reserva **280–283** para a F7 (F8 = 290–295). As 4 migrations do plano (280/281/282/283) cabem exatamente, mas a frase "284–289 ficam livres para desdobramentos" (Autorrevisão) **não vale mais** — desdobramento extra exige negociar faixa |
| P2 | `utils/autorizacao.py` com `obras_visiveis()`, `pode_ver_obra()`, `obra_required` | **CONFIRMADA** | `utils/autorizacao.py:73` (`obras_visiveis`), `:145` (`pode_ver_obra`), `:184` (`obra_required`). Bônus da F1.5: `pode_editar_obra` (`:149`) e `pode_apontar_na_obra` (`:153`), já consumidos pelos guards do cronograma (`cronograma_views.py:53` `_guard_editar_obra`, `:72` `_guard_apontar_obra`) | Tasks 5/6/7: usar os guards **por verbo** que as rotas vizinhas já usam (`pode_ver_obra` para leitura, `pode_editar_obra` para escrita), não só `obra_required` genérico — ver N1 |
| P3 | BAC = previsto vivo até a Fase 6 versionar orçamento | **CONFIRMADA** | Fase 6 continua ⬜ (`ESTADO-ATUAL.md`, tabela do plano aprovado); `_previsto_por_categoria` segue em `services/cronograma_fisico_financeiro.py:200` | Nenhum. A dependência declarada em D3 segue valendo |
| P4 | O plano `cronograma-editavel-rdo-percentual.md` "ainda não existe" | **QUEBRADA — executada (favorável)** | A Fase 1.5 fechou em 22/07 (14/14 tasks). `modo_apontamento` virou coluna (`models.py:5515`, migrations 220/221); `modo_da_tarefa()` em `services/cronograma_apontamento_service.py:111` (a dedução citada em `:73` virou `_modo_deduzido`, `:95`). O contrato temido **não quebrou**: `calcular_progresso_rdo` (`utils/cronograma_engine.py:429-507`) continua devolvendo `percentual_realizado` float — e **melhorou**: tarefa sem `quantidade_total` agora devolve o percentual do último apontamento (`:477-497`; antes devolvia sempre 0) | O EV da Task 8 fica **mais correto** para tarefa em modo percentual (inclusive esqueleto). Acrescentar aos testes da Task 8 um caso de obra com tarefas em modo `percentual` (o coletor de VA deve refletir o último apontamento, não 0) |
| P5 | Nomes dos estados da Obra (Fase 2) | **CONFIRMADA — respondida** | `EstadoObra` em `models.py:29-56`: `PLANEJAMENTO`, `EM_EXECUCAO`, `PAUSADA`, `CONCLUIDA`, `CANCELADA` (VARCHAR+CHECK, migration 231). O plano dizia "não invente `EM_EXECUCAO`" — é exatamente o nome real | Task 6 já pode escrever a guarda com nomes reais. Ler `Obra.estado`, **não** `Obra.status` (virou espelho write-through da F2) — ver N4 |
| P6 | Medir quantas obras têm `ItemMedicaoCronogramaTarefa` | **ABERTA, com nuance favorável** | Acesso ao banco de produção segue travado (`ESTADO-ATUAL.md`, item humano); o banco dev foi **recriado em 22/07** — a volumetria antiga não vale nem como forma. Nuance: `materializar_cronograma` cria o vínculo com peso para **toda** obra materializada de proposta (`services/cronograma_proposta.py:730-773`), **inclusive tarefa-esqueleto** (`:649-653` — a raiz vira folha com 100% do peso do item) | O universo "sem vínculo ⇒ fallback físico" é menor que o temido: essencialmente obras nascidas só de `.mpp`. A medição em produção continua pré-requisito antes de priorizar uma tela de vínculo |
| P7 | Flag própria vs. herdar `cronograma_mpp_ativo` | **CONFIRMADA** | `cronograma_mpp_ativo` agora em `models.py:3937` (era `:3620`); `utils/tenant.py:113` intacto. Há agora **três** precedentes-irmãos: `escopo_obra_ativo` (`models.py:3945`, F1) e `compras_governanca_ativa` (`:3955`, F3), cada um com script próprio | Nenhum — flag própria (Task 1) segue sendo a decisão certa, e o molde a copiar é o mais recente (`scripts/flag_compras_governanca.py`, que já recusa precondição ausente) |
| P8 | Asserts `pct_fisico is None` em `:136` e `:338` | **CONFIRMADA** | `tests/test_importacao_fisico_financeiro.py:136` e `:338` intactos, nas mesmas linhas | Nenhum — a Task 9 os atualiza como planejado |
| P9 | `additionalProperties: false` no schema + `NORMALIZADOR_VERSAO` | **CONFIRMADA** | `services/schemas/cronograma_normalizado.schema.json:30` e `:57` intactos; `NORMALIZADOR_VERSAO = '1.0'` mudou de `:25` para `services/cronograma_normalizacao.py:27` | Nenhum |

### Premissas implícitas das seções 1, 2, 6 e 7

| Afirmação do plano | Veredito | Evidência (23/07) | Ajuste necessário |
|---|---|---|---|
| Seção 2: "`Obra.regime_medicao` — nenhuma view ou service a lê" | **QUEBRADA** | A F1.5 passou a lê-la: `cronograma_views.py:507-508` usa `obra.regime_medicao == 'percentual'` como default de `modo_apontamento` de tarefa nova; o comentário do modelo foi reescrito (`models.py:405-427`; a coluna mudou de `:292` para `:427`) | Sem impacto nas tasks (nenhuma dependia da coluna estar morta), mas a linha da tabela da seção 2 está falsa. Não "adotar" a coluna para o EVM sem ler o comentário novo |
| Seção 1/2: delta real — sem `TarefaPredecessora`, sem baseline, sem EVM | **CONFIRMADA** | `grep` em 23/07: zero ocorrências de `TarefaPredecessora`/`tarefa_predecessora`, `is_baseline`/`baseline` e `valor_agregado`/`earned` em `*.py` fora de `docs/` | Nenhum — o delta da fase continua sendo exatamente o planejado |
| Só a 1ª predecessora FS sobrevive à aplicação de versão | **CONFIRMADA** | `services/cronograma_versao_service.py:103` (`_primeira_fs`), `:586` (uso); `predecessoras_json` no snapshot (`models.py:5743`) e em `:146`, `:801`, `:828` do serviço | Nenhum |
| Motor: `utils/cronograma_engine.py` — todas as âncoras de linha | **CONFIRMADA (raro)** | Arquivo continua com **888 linhas** e TODAS as refs batem: `dias_uteis_entre :73`, `get_calendario :89`, `verificar_ciclo :108`, `_planejado_na_data :143`, `recalcular_cronograma :297`, `calcular_progresso_rdo :429`, `calcular_progresso_geral_obra_v2 :510`, `rollup_realizado :632`, `replanejar_curvas_obra :724`. A F1.5 pôs a lógica de modo em serviço separado, não no engine | Nenhum |
| `services/cronograma_fisico_financeiro.py` — âncoras | **CONFIRMADA** | `:25` fasear, `:62` alocar_por_peso, `:86` montar_curva_s, `:200` previsto, `:254` `pct_fisico: None`, `:482` painel_financeiro, `:538` repasse, `:563` curva_realizado — todas exatas | Nenhum |
| `_motor_pos_commit` / `restaurar_versao` | **CONFIRMADA** | `services/cronograma_versao_service.py:430` e `:668`/`:691`, exatos | Nenhum |
| Line refs de `models.py` (seções 1, 2, 6, Tasks 2/4/6) | **QUEBRADA (drift, sem mudança semântica)** | `models.py` cresceu ~600 linhas (F1/F2/F3). Novos endereços: `TarefaCronograma :5461` (era :4863; termina ~`:5666`, não :4952), `predecessora_id :5481`, **novo** `modo_apontamento :5515`, `percentual_planejado :5605`, `CronogramaVersao :5668` (comentário do unique parcial da migration 207 preservado no docstring), `CronogramaTarefaSnapshot :5711`, `predecessoras_json :5743`, `CronogramaImportacaoEvento :5799`, `GestaoCustoFilho :5914`, `ItemMedicaoCronogramaTarefa :6087`, `valor_contrato :346`, `token_cliente :372` (+ `token_cliente_expira_em :379`, F3) | Ao executar, localizar por **nome de classe/coluna**, não pela linha do plano |
| Line refs de views/templates (Tasks 5/6/7/9) | **QUEBRADA (drift)** | `cronograma_views.py`: `_check_v2 :39` (igual), `_tarefa_to_dict :86` (era :53 — e **já serializa `modo_apontamento`**, `:105`), `cronograma_obra :235` (era :164), `criar_tarefa :340` (era :269). `views/obras.py`: `financeiro_dados :2204-2207` (era :2089), `curva-avanco :2678` (era :2555). `views/cronograma_importacao.py`: `listar_versoes :629` (era :627). `templates/obras/cronograma.html`: 2.508 linhas (era 2.465), `renderGantt :2171` (era :2129), cabeçalho `Pred.` intacto em `:116`. `static/js/financeiro_obra.js`: chart de 4 séries em `:385` | Idem — localizar por nome. A coluna Folga (Task 5) entra ao lado de `Pred.` em `:116`, como planejado |
| `decorators.py:7` (`cronograma_import_required`), `run_tests.sh --gate/--java`, `tests/test_cronograma_multitenancy.py`, `tests/test_cronograma_permissoes.py` | **CONFIRMADA** | `decorators.py:7` exato; `run_tests.sh:36` (`--gate`) e `:49` (`--java`); os dois arquivos de teste existem (22 arquivos `test_cronograma_*`) | Nenhum |
| Parsers e normalizador intocados | **CONFIRMADA** | `services/mspdi_parser.py:44` (`_TIPO_VINCULO`), `:47` (`_LAG_POR_DIA`), `:112-113`; `services/mpp_parser_worker.py:50` (`_lag_dias`), `:116`; `services/cronograma_normalizacao.py:209` (`_detectar_ciclos`), `:357-358` (tipo/lag preservados) | Nenhum |

### Riscos NOVOS (criados pelas Fases 1–3, não previstos no plano)

| # | Risco | Evidência | Ajuste necessário |
|---|---|---|---|
| N1 | **O RBAC não é mais hipotético: `escopo_obra_ativo` já está LIGADA em 21 tenants** (medição dev de 22/07, `ESTADO-ATUAL.md`). O plano tratava a Fase 1 como futura (P2 falava em "assinatura"); agora as rotas do cronograma têm guards reais por verbo (`_guard_editar_obra`/`_guard_apontar_obra`, `cronograma_views.py:53/:72`) | `cronograma_views.py:53-84`; `utils/autorizacao.py:145-183` | Tasks 5/6/7 devem declarar o guard **por verbo**: leitura de caminho crítico/comparativo ⇒ `pode_ver_obra`; congelar baseline ⇒ `pode_editar_obra` (ou papel GESTOR explícito, D5). Testar com a flag LIGADA e DESLIGADA — a lição da F1 (teste que só exercita um lado esconde regressão) |
| N2 | **`recalcular_cronograma` ganhou um caller onde exceção PROPAGA de propósito.** A materialização de proposta chama o engine e documenta "falha aqui DEVE propagar — sessão será revertida pelo caller" (`services/cronograma_proposta.py:775-782`, F1.5). O hook de CPM da Task 4 no fim de `recalcular_cronograma` herdaria esse caller: um bug no CPM reverteria a **materialização da obra inteira**, dano maior que o previsto (o plano só considerava aplicação de versão) | `services/cronograma_proposta.py:775-782` | O `try/except` do hook (Task 4) passa de recomendação a **obrigação absoluta**, com teste dedicado: CPM que levanta exceção não pode impedir `materializar_cronograma` de commitar |
| N3 | **Tarefas-esqueleto entram na rede do CPM.** Item de proposta sem template agora vira tarefa nível 0, folha, `data_inicio=data_seed`, `duracao_dias>=1`, sem nenhuma aresta (`services/cronograma_proposta.py:621-636`, F1.5) | `services/cronograma_proposta.py:580-587` (o `continue` removido), `:649-653` | Obra recém-materializada só de esqueletos sairá do CPM com **todas as tarefas críticas** (comportamento definido na Task 4 para rede sem arestas — correto, mas vazio de informação). Task 5: quando a obra não tem NENHUMA aresta, a UI deve dizer "sem rede de precedência — caminho crítico não informativo" em vez de pintar tudo de crítico |
| N4 | **A Obra tem máquina de estados (F2) e `status` virou espelho.** `Obra.estado` (VARCHAR+CHECK, migrations 230-232) é a fonte; `Obra.status` é derivado por write-through (docstring de `EstadoObra`, `models.py:43-46`) | `models.py:29-56` | A guarda de estado da Task 6 (P5) lê `Obra.estado` e, se bloquear transição, respeita a máquina da F2 (trilha `obra_transicao_estado`) — não escreve em `status` |
| N5 | **`PapelObra` ganhou `COMPRADOR` (migration 245) e há o colapso conhecido com escopo OFF.** Achado nº 1 do review da F3: com `escopo_obra_ativo` desligada, `papel_na_obra` devolve GESTOR a todo autenticado — a guarda de papel do rebaseline (D5) colapsaria igual à alçada da F3 | `models.py:59-77`; `utils/autorizacao.py:102`; `ESTADO-ATUAL.md` (review F3, achado 1) | Task 6: (a) matriz de papéis nos testes inclui os **4** papéis (COMPRADOR não rebaselina); (b) documentar a mesma dependência dura da F3 — rebaseline por papel só é efetivo com `escopo_obra_ativo` ligado no tenant; o script da flag da F7 pode espelhar a recusa do `flag_compras_governanca.py` |
| N6 | **Fase 4 em andamento em paralelo** (worktree `.claude/worktrees/fase-4`, untracked em `main`), consumindo as migrations 250-254 | `.claude/worktrees/fase-4/migrations.py:5105-5109` | Sem colisão com 280-283 hoje. Reconfirmar P1 **no dia** de escrever a migration 280 — a fila anda |

### Impacto no plano

**A Fase 7 segue executável como desenhada — nenhuma tarefa cai, nenhuma decisão D1–D7 muda.** A arquitetura apostou nas âncoras certas: o motor (`cronograma_engine.py`), o serviço de versão, o físico-financeiro, os parsers e o normalizador estão **byte-a-byte nas mesmas linhas** do plano, e o delta (sem aresta tipada, sem baseline, sem EVM) continua exato. As quebras são de dois tipos: (1) drift de linha em `models.py`/views/templates — resolve-se localizando por nome; (2) premissas que o tempo **respondeu a favor** — P4 (o contrato do progresso melhorou para modo percentual), P5 (os nomes dos estados existem) e a nuance de P6 (esqueletos nascem com vínculo de peso).

O que a execução deve incorporar além do texto original: os guards por verbo da F1.5 nas rotas novas testados com a flag ligada E desligada (N1); `try/except` blindado no hook de CPM porque a materialização agora propaga exceção de propósito (N2); mensagem de "sem rede de precedência" para obra só-esqueleto (N3); `Obra.estado` + trilha da F2 na guarda de baseline (N4); e a matriz de 4 papéis com a dependência dura de `escopo_obra_ativo` documentada no molde da F3 (N5). A faixa 280-283 está livre (maior aplicada: 247), mas ela comporta **exatamente** as 4 migrations do plano — a folga "284-289" citada na Autorrevisão não existe mais na reserva do `ESTADO-ATUAL.md`, e a Fase 4 já corre em paralelo consumindo 250-254: reconfirmar a faixa no dia de escrever a 280.
