...
## Revisão de premissas — 2026-08-23 (pós-p9/p10 e Fases 4-5 do ciclo)

> Reconferência integral contra `main` em `b97ec6ad` (branch `feat/regua-status-unificado`). Desde a revisão de 03/08 (`a2d3dc02`) entraram, entre outros: p10 completo (`services/evm.py`, `CronogramaBaseline.bac`, e agora `CronogramaBaseline.revisao`/`.motivo` — migration 314, 21/08), o E05 do CRM, a Fase 2 do ciclo de compras (nota fiscal + adiantamento, migration 311), a Fase 3 (alçadas avançadas, migrations 297-299), a Fase 4 do ciclo de compras (recebimento/atesto, migrations 283-286), e a régua de status unificado do módulo de compras (`services/etapa_compra.py`, sem coluna nova — 100% derivada). Nenhum desses pacotes toca `ObraContratoVersao`, `AditivoContrato`, `Orcamento` ou `services/contrato_obra.py` — a superfície da Fase 6 continua isolada, mas a numeração de migration e os números de linha de `models.py` seguiram andando por baixo dela. Maior migration registrada hoje: **314**. Gate de referência (`ESTADO-ATUAL.md`, 23/08): **2560 passed, 6 skipped, 2 xfailed**, zero falhas.

### Premissas numeradas (§7)

| # | Premissa | Veredito | Evidência (23/08) | Ajuste necessário |
|---|---|---|---|---|
| 1 | `valor_contrato` escrito só em `event_manager.py:1043`, `views/obras.py:391` e `:873` | **CONFIRMADA (sem drift desde 03/08)** | O escritor único do p9 segue de pé e é o único caminho: `services/contrato_obra.py:57` (`definir_valor_contrato`). Chamadores hoje: `event_manager.py:1367` (criação de obra) e `:1435` (revisão/aditivo, dentro do `else` que também congela `MedicaoContrato.valor_base`), `views/obras.py:317` (criação) e `:880` (edição), `services/importacao_fisico_financeiro.py:757` — **5 pontos de chamada**, todos passando por `definir_valor_contrato`. Nenhum escritor direto de `obra.valor_contrato =` sobrou fora do módulo (`grep -rn "valor_contrato\s*="` só bate dentro de `contrato_obra.py` e nos testes) | Nenhum. A D6 (escritor único) está pronta há 20 dias e não regrediu. **Mas o contrato ainda não é o da Task 2**: não existem `contrato_vigente()`, `valor_vigente_em()` nem `abrir_versao()` — `definir_valor_contrato()` grava o `Float` e loga origem/motivo, não abre `ObraContratoVersao`. Task 2 vira "estender esta função", não "criar do zero" |
| 2 | A duplicação de IMC/lançamento de §1.1 ainda acontece | **QUEBRADA (sem mudança desde 23/07)** | `handlers/propostas_handlers.py:54-176`: reconciliação por linhagem intacta — item existente é **atualizado** (`existente.valor_comercial = valor_total`, `:129-134`), item novo é criado, item sumido vira só **WARNING logado** (`:151-165`), nunca `DELETE`. `:410-459`: lançamento contábil segue lançando o **delta** (`ja_lancado` somado por `origem == 'PROPOSTAS'` na linhagem, `:421`), com `4.2.01.001` de dedução no estorno | Nenhum. Segue valendo a leitura de 23/07: Tasks 7 e 8 são "completar", não "criar" |
| 3 | `handlers/propostas_handlers.py` é o único criador de IMC a partir de proposta | **CONFIRMADA** | Único `ItemMedicaoComercial(` a partir de proposta continua em `handlers/propostas_handlers.py:135` (bloco de criação). `_semear_servicos_reais` (p5, ainda presente) cria `ServicoObraReal`, não IMC | Nenhum |
| 4 | A faixa 271-276 está livre e o 270 continua queimado | **CONFIRMADA — mas a vizinhança ficou mais barulhenta** | `grep "_migration_27[0-6]_" migrations.py` → **zero ocorrências**: 271-276 seguem 100% livres. Maior migration registrada hoje é a **314** (`_migration_314_baseline_revisao_e_motivo`, cronograma). A lista `migrations_to_run` (agora em `migrations.py:7026`) tem furos deliberados e documentados: 290-295 reservada para uma Fase 8 que não existe mais como tal, 300-307 para uma Fase 9, nenhuma das duas aplicada — e a 287 foi **renumerada para 311** em 19/08 porque colidiu com outra linhagem do repo (`9a8381ec`) | Nenhum na faixa da própria fase. Mas **registre o novo precedente de colisão** (a 287→311) no Step 2 da Task 14: antes de reivindicar 271, rode o mesmo grep que este item usou — a lista já mostra que "faixa reservada" não impediu duas colisões (270 em 03/08, 287 em 19/08) neste projeto |
| 5 | `MedicaoContrato` continua sem UI de escrita | **CONFIRMADA** | Único criador segue o import: `services/importacao_fisico_financeiro.py:757` (o mesmo ponto que agora também chama `definir_valor_contrato`). `grep -rn "MedicaoContrato" views/` → nada | Nenhum. Mas o modelo (`models.py:7516`) ainda não tem `contrato_versao_id` — a Task 4 segue redesenhada sobre `valor_base` (N8 da revisão de 23/07), inalterada |
| 6 | Nome real do decorator de autorização por obra | **CONFIRMADA** | `utils/autorizacao.py:170` `papel_na_obra`, `:190` `pode_ver_obra`, `:194` `pode_editar_obra`. Não existe `obra_required` como decorator de rota — as revisões anteriores já citavam esses três nomes; confirme por nome, não assuma o decorator do plano original | Usar o guard funcional (`pode_editar_obra`/`papel_na_obra`), como já apontavam as revisões de 23/07 e 03/08 |
| 7 | Fase 4 (centro de custo) não tornou o campo obrigatório sem default | **CONFIRMADA** | `centro_custo_id` segue nullable em todos os pontos relevantes: `PartidaContabil` (`models.py:3352`, sem `nullable=False`), `pedido_compra` (`:5756`, `nullable=True` explícito). `ObraServicoCusto` (`:7784-7885`) **não tem** coluna `centro_custo_id` — a F4 nunca alcançou essa tabela | Nenhum. Os inserts das Tasks 7 e 8 não quebram |
| 8 | `Obra.regime_medicao` continua `'fixa'\|'percentual'` | **CONFIRMADA — sem mudança de forma desde 03/08** | `models.py:452` (era `:427` em 03/08 — deslocou pelas ~25 linhas que a F2/estado acrescentou acima). Continua sendo lida por `cronograma_views.py` para o default de `modo_apontamento` | Localizar por `grep "regime_medicao ="`, não por linha fixa — a coluna já se moveu duas vezes desde julho |
| 9 | `criar_nova_versao` é a única porta para revisar proposta enviada | **CONFIRMADA (drift de linha)** | `propostas_consolidated.py:1350` (era `:1248` em 03/08). Gate de `rascunho` ainda existe antes da rota aceitar edição direta | Localizar por nome (`def criar_nova_versao`), não por linha |
| 10 | `TarefaCronograma.ativa` continua existindo, sem semântica conflitante | **CONFIRMADA — mas ganhou uma prima homônima nova** | `TarefaCronograma` agora em `models.py:6545` (drift). `CronogramaBaseline` (`models.py:6783`) também tem coluna `ativa` (a "uma baseline ativa por obra/modo") — já eram duas colunas `ativa` desde 03/08, mas agora `CronogramaBaseline` **também** ganhou `revisao` e `motivo` (migration 314, 21/08), com o próprio docstring do modelo usando **"Aditivo 01"** como exemplo de motivo de revisão de baseline | Ver risco novo N7 abaixo — a palavra "aditivo" já aparece livre-texto num campo de cronograma que não é o `AditivoContrato` da Fase 6 |

### Riscos NOVOS desde 03/08

| # | Risco | Evidência | Consequência para a fase |
|---|---|---|---|
| N7 | **Vocabulário "aditivo" já vaza para o cronograma, sem estrutura.** `CronogramaBaseline.motivo` (`models.py:6817-6820`, migration 314) é texto livre e o próprio comentário do modelo usa `'Aditivo 01'` como exemplo — alguém já está digitando "aditivo" numa baseline de cronograma para marcar a revisão que segue um aditivo contratual, porque não existe `AditivoContrato` para apontar | `models.py:6783-6845`; commit `37708061` | D3 (aditivo re-materializa cronograma, incremental) precisa decidir se `aprovar_aditivo()` da Fase 6 **cria automaticamente** uma nova `CronogramaBaseline` com `motivo=f'Aditivo {numero}'` e `revisao` sequencial — hoje isso seria feito à mão pelo operador, exatamente a lacuna que motivou a migration 314. Vale registrar como decisão nova (D9?) em vez de deixar a integração implícita |
| N8 | **`CronogramaBaseline.bac` (p10, `models.py:6833-6845`) e o EVM (`services/evm.py`) já dependem do orçamento congelado — mas não do contrato.** BAC é lido de `CronogramaBaseline.bac`, não de `ObraContratoVersao.valor` nem de `Obra.valor_contrato` | `models.py:6833-6845`; N3 da revisão de 03/08, ainda não resolvida | Confirma que N3 (03/08) segue em aberto: a Fase 6 precisa decidir explicitamente se `abrir_versao`/`aprovar_aditivo` deve **também** congelar um novo BAC de baseline, ou se as duas "verdades congeladas" (contrato vigente vs. BAC da baseline) coexistem por design. Nenhuma decisão nova tomada desde 03/08 — está pendente, não resolvida |
| N9 | **Migration renumerada pela segunda vez no projeto** (287→311, `9a8381ec`, 19/08), pelo mesmo motivo da 270→277 (colisão de linhagem de branches). O padrão de "faixa reservada previne colisão" já falhou duas vezes | `migrations.py:6345` (`_migration_311_nota_e_adiantamento`, comentário "ERA 287") | Reforça a premissa 4: a checagem de "maior migration + faixa livre" tem de ser feita **no momento de escrever código**, não confiar na reserva feita em julho. Nenhuma mudança na faixa 271-276 em si |
| N10 | **A régua de status de compras (`services/etapa_compra.py`, branch atual) é 100% derivada — zero coluna nova.** Não colide com nada da Fase 6, mas é o precedente mais recente de "campo calculado sem escrita própria" no repo, na mesma família de design que D6 (`valor_contrato` como cache derivado) | `ESTADO-ATUAL.md:2405-2416`; `services/etapa_compra.py` | Nenhum ajuste — citado só porque reforça que D6 (cache derivado, escritor único) é o padrão que o resto do sistema também está adotando agora, o que sustenta manter a decisão como está |

### Impacto no plano

**A Fase 6 segue executável, isolada das Fases 4-5 do ciclo de compras, e não encolheu mais desde 03/08 — mas também não avançou nenhuma task nova.** Nenhum dos componentes que faltam (`ObraContratoVersao`, `AditivoContrato`, cadeia de revisão do `Orcamento`, comparador, trava de edição do orçamento convertido, fechamento da edição livre de `valor_contrato`) foi tocado nos últimos 20 dias. O que mudou é o terreno ao redor: a faixa de migração livre encolheu de cima (maior registrada foi de 278 para 314) sem invadir 271-276, e o cronograma ganhou uma segunda noção de "revisão com motivo" (baseline) que usa a palavra "aditivo" informalmente — um convite a integrar (N7) que o plano original não previa porque a coluna não existia em 21/07.

Números de referência atualizados: gate = **2560 passed, 6 skipped, 2 xfailed** (23/08, não mais 878 nem 1122 nem 1778); maior migration = **314**; `migrations_to_run` agora em `migrations.py:7026`; `Obra` agora começa em `models.py:360` (`valor_contrato` `:371`, `status` `:375`, `estado` `:384-389`, `proposta_origem_id` `:412`, `regime_medicao` `:452`); `Proposta` em `:3710` (`cadeia_versoes` `:3886`); `PropostaItem` em `:3956`; `TarefaCronograma` em `:6545`; `ItemMedicaoComercial` em `:7381` (UNIQUE `proposta_item_id` `:7396`); `MedicaoContrato` em `:7516` (`valor_base` presente, sem `contrato_versao_id`); `Orcamento` em `:8456`/`OrcamentoItem` em `:8504` (sem `versao`/`origem_id`/`travado_em`/`item_origem_id` — Tasks 10-12 valem 100% como escritas); `ObraOrcamentoOperacional`/`ItemVersao` em `:8874`/`:8929`.

### Tasks já entregues ou alteradas pelo p9/p10 (e pelo que chegou depois de 03/08)

| Task | Situação | Evidência |
|---|---|---|
| **Task 1** — `ObraContratoVersao` + migration 270/271 + backfill | **intacta (nada entregue)** | `grep -n "class ObraContratoVersao" models.py` → zero resultados. Não existe a tabela, não existe backfill. Continua 100% a fazer, na faixa 271-276 |
| **Task 2** — `services/contrato_obra.py`, leitura e escritor único | **parcialmente entregue — a metade "escritor único" está pronta desde 03/08 (p9), a metade "versionamento" não existe** | `services/contrato_obra.py` existe e é chamado pelos **5 pontos** que a Task 2 precisava fechar (`event_manager.py:1367`,`:1435`; `views/obras.py:317`,`:880`; `services/importacao_fisico_financeiro.py:757`) — mas o contrato de função implementado é só `definir_valor_contrato(obra, valor, origem, motivo, usuario_id)`. As funções que a Task 2 mandava criar — `contrato_vigente()`, `valor_vigente_em()`, `abrir_versao()` — **não existem**. A Task vira: manter `definir_valor_contrato` como ponto de entrada e, **dentro dela**, abrir/fechar `ObraContratoVersao` sem quebrar os 5 chamadores existentes |
| **Task 3** — `AditivoContrato` + migration 271 | **intacta (nada entregue)** | `grep -n "class AditivoContrato" models.py` → zero. Nenhum modelo, nenhuma transição `rascunho→aprovado`/`cancelado` |
| **Task 4** — `MedicaoContrato` presa à versão, migration 272 | **parcialmente entregue por outro mecanismo (desde 23/07, sem mudança)** | `MedicaoContrato.valor_base` (`models.py:7534`) já congela a base no aditivo (`event_manager.py:1403-1420`, dispara quando `anterior > 0` e há medição `recebido_no_mes` preenchida). Não existe `contrato_versao_id`. Como já registrado em 23/07 (N8): a FK vira rastreabilidade, o mecanismo de imutabilidade real já está em produção — falta só o **repontamento explícito com preview** que é o entregável de negócio da Task 4 |
| **Task 5** — Fechar edição livre de `valor_contrato` | **intacta (nada entregue)** | `views/obras.py:875-885`: o POST de `editar_obra` ainda aceita qualquer `valor_contrato` do formulário e grava via `definir_valor_contrato(origem=ORIGEM_EDICAO)` **sem checar** se existe contrato vigente. Nenhuma trava, nenhum badge, nenhum link para "aditivos" no template |
| **Task 6** — Elo item↔item + campos dimensionais na revisão de proposta | **parcialmente entregue, sem mudança desde 07-23** | A linhagem já existe por outro nome (`PropostaItem.proposta_item_origem_id`, migration 219, semântica de raiz — `propostas_consolidated.py:1437`). Os 6 campos dimensionais (`tipo_medicao_override`, `dim_*`) **continuam ausentes** do bloco de clonagem (`propostas_consolidated.py:1428-1456`) — comparados hoje contra `views/orcamentos_views.py:568-573`, que **já copia** esses mesmos campos na revisão de Orçamento. A Task 6 encolhe para "copiar os 6 campos", exatamente como a revisão de 07-23 previu |
| **Task 7** — Propagação reconciliadora | **parcialmente entregue (núcleo pronto desde 0.6/07-23, faltam os dois refinamentos)** | Reconciliação por linhagem funciona (ver premissa 2 acima). Falta: (a) trocar o `WARNING` de item sumido (`handlers/propostas_handlers.py:151-165`) por `status='SUPRIMIDO'` gravado no IMC — hoje é só log; (b) a guarda de "aditivo não pode reduzir item já medido abaixo do executado" — não há nenhuma checagem de `percentual_executado_acumulado` neste arquivo (`grep` não achou o termo) |
| **Task 8** — Contabilidade do aditivo, só o delta | **substancialmente entregue, com desenho diferente do D5 original — sem mudança desde 07-23/03-08** | Delta lançado com conta de dedução `4.2.01.001` (não inversão de partidas). `origem` do lançamento **continua `'PROPOSTAS'`**, não `'ADITIVO'` — resolvendo N2 (03/08) ao manter a query de "já lançado" funcionando por linhagem em vez de trocar o rótulo. Falta apenas: gravar a referência ao `AditivoContrato.id` quando ele existir (hoje `origem_id` aponta para `proposta_id`) |
| **Task 9** — Cronograma no aditivo, incremental | **intacta na parte de negócio; terreno mudou por baixo (N7/N8 acima)** | `TarefaCronograma.ativa` segue existindo e sem uso conflitante. Nenhuma lógica de "item suprimido → `ativa=False`" foi implementada no caminho de aditivo (não existe aditivo ainda). Mas `CronogramaBaseline` ganhou `revisao`/`motivo`/`bac` desde 03/08 — a Task 9 precisa decidir se acopla ao ciclo de baseline (N7) em vez de operar só sobre `TarefaCronograma.ativa` como o plano original assumia |
| **Task 10** — Cadeia de revisões do Orçamento, migrations 274/275 | **intacta (nada entregue)** | `models.py:8456-8503` (`Orcamento`): sem `origem_id`, `revisao_de_id`, `versao`, `motivo_revisao`, `travado_em`. `duplicar()` (`views/orcamentos_views.py:533-588`) continua criando cópia **órfã**, sem FK de volta — idêntico ao verificado em 21/07 e 23/07 |
| **Task 11** — Trava de edição do orçamento convertido | **intacta (nada entregue)** | As seis rotas (`atualizar` `:288`, `adicionar_item` `:315`, `atualizar_item`, `remover_item`, etc.) não checam `status`/`travado_em` — a coluna nem existe. `gerar_proposta` continua só setando `status`, sem travar |
| **Task 12** — Diff/comparador + migration 276 | **intacta (nada entregue), e a nota de 23/07 sobre "rediscutir ou eliminar" (N3) segue válida** | Nenhum `services/proposta_diff.py`, nenhuma rota `/comparar`. A linhagem por raiz (`proposta_item_origem_id`) já dá o pareamento sem precisar da migration 276 — a recomendação de 23/07 (usar o fallback em runtime e matar a 276) não foi nem confirmada nem descartada, porque a Task não começou |
| **Task 13** — UI do aditivo, extrato de contrato | **intacta (nada entregue)** | Nenhum blueprint `views/aditivos_views.py`, nenhum template. Depende de Task 1/3 existirem primeiro |
| **Task 14** — Fecho da fase | **não aplicável ainda** — mas o número de referência do gate mudou para **2560 passed** (23/08), não mais 878/1122/1778 |
