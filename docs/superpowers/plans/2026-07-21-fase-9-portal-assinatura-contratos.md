...
Releitura integral contra o código de `main` em 23/07 (commit `8948573a` — Fases 1, 1.5, 2 e 3 mergeadas; rodada de pendências de 23/07 incluída; migrations aplicadas até **247**). O plano foi escrito em 21/07 sobre o commit `fb4147b`; desde então a **Fase 3 mexeu exatamente no objeto desta fase** (o portal por token) e a Fase 0.5/23-07 acrescentou uma rota nova ao portal. Nada disso invalida a arquitetura da 9a — mas muda a forma de várias tasks. Vereditos: **CONFIRMADA** (vale como escrito, evidência de hoje), **QUEBRADA** (o código mudou; ajuste indicado), **NOVA** (risco/condição que não existia em 21/07).

### Tabela de premissas

| # | Premissa do plano | Veredito | Evidência (23/07) | Ajuste necessário |
|---|---|---|---|---|
| R1 | "O token não expira. Nunca. Não há coluna de validade" (S2; contexto §"O portal hoje") | **QUEBRADA** | A migration **247** (F3) criou `obra.token_cliente_expira_em` (`models.py:379`; `migrations.py:4468-4507`). `toggle_portal` carimba **+180 dias** a cada reativação (`portal_obras_views.py:674-684`; `PRAZO_TOKEN_DIAS = 180` em `:49`). Os dois helpers checam expiração e devolvem **404** (`portal_obras_views.py:71-75` e `:87-90`). Token sem data (pré-247) segue valendo — deliberado | Task 1/Task 4: o backfill (migration 302) **não pode carimbar 90 dias chapados** — deve **herdar `token_cliente_expira_em` quando preenchido** (senão ENCURTA validade que a F3 acabou de prometer) e só aplicar o default aos sem data. Task 8 (migration 305): além de zerar `token_cliente`, aposentar `token_cliente_expira_em`, `PRAZO_TOKEN_DIAS` e o carimbo do `toggle_portal` |
| R2 | D2 recomenda validade default de **90 dias** | **QUEBRADA** | A decisão já foi tomada na F3: **180 dias** (`ESTADO-ATUAL.md:570` — "o plano da F9a dizia 90; a F3 escolheu 180 para não gerar chamado de suporte a cada obra") | `portal_token_dias_validade` (migration 304) nasce com default **180**, não 90. O token de assinatura continua 7 dias (não afetado) |
| R3 | "Não há trilha. Nenhuma das 7 rotas grava quem, quando, de onde" (S5) | **QUEBRADA** (parcialmente) | Existe `PortalAcessoEvento` (`models.py:5355-5390`, tabela `portal_acesso_evento`, migration 247) com obra_id, admin_id, ação, alvo, **IP, user-agent**, detalhes JSON. Gravada por `_registrar_acesso` (`portal_obras_views.py:104-128`) em todo POST do portal de obra e no `ver_comprovante` (`:431`, `:513`, `:618` etc.). Não identifica pessoa (não há como sem identidade — é a 9a); identifica origem | Task 3 **não cria uma segunda tabela de trilha**: generaliza/estende `portal_acesso_evento` (acrescentar `acesso_id` FK para `portal_acesso`, `escopo`, `resultado` concedido/negado) ou faz a migration 301 virar ALTER + rename — mesmo tratamento condicional que a Task 9 já prevê para `rdo_assinatura`. As rotas de proposta (`propostas_consolidated.py`) continuam **sem** trilha: ali a Task 3 segue integral |
| R4 | "Nenhum arquivo em `tests/` exercita o blueprint `portal_obras`" | **QUEBRADA** | `tests/test_fase3_portal_seguranca.py` (9 testes: expiração, token sem data, carimbo do toggle, trilha IP, ciência sob governança) e `tests/test_fase06_d2_portal_compra.py` | Tasks 5/8/14 não podem quebrá-los. **Conflito direto a resolver**: a F3 pinou token expirado → **404** (`test_token_expirado_nao_abre_o_portal`, `tests/test_fase3_portal_seguranca.py:115-124`); o plano quer **410** com página explicativa (critério de pronto 9a). Ao aplicar a Task 5, atualizar os testes da F3 junto (o 410 é superação consciente, não regressão) — ou manter 404 e ajustar o critério. Decidir explicitamente no fecho |
| R5 | Task 5 aplica o decorator às **8 rotas** de `portal_obras_views.py` (11 no total com propostas) | **QUEBRADA** | O portal ganhou uma **9ª rota por token**: `ver_comprovante` GET `/obra/<token>/compra/<id>/comprovante` (`portal_obras_views.py:607-619`), criada em 23/07 substituindo `/persistent-uploads` (triagem Anexo B, commit `8948573a`) | Task 5 inclui `ver_comprovante` na matriz (escopo `LER` ou `ENVIAR_COMPROVANTE` — recomendado `LER`, é leitura). A varredura do `url_map` (Task 14) e `tests/test_fase9_portal_rotas.py` passam a cobrir **12 rotas por token** (9 portal_obras + 3 propostas... + `portal_pdf_extrato` = 13 endpoints no total) |
| R6 | `aprovar_compra` chama `processar_compra_aprovada_cliente` direto; corpo simples a decorar ("troque a primeira linha do corpo") | **QUEBRADA** | O corpo hoje tem 3 camadas que não existiam em 21/07: (1) `_get_compra_do_portal` (F0.6/D2, `portal_obras_views.py:401-422`) escopando tenant+`tipo_compra='aprovacao_cliente'`; (2) guarda de transição `_STATUS_COMPRA_DECIDIVEL` (`:444`, `:522`); (3) ramo de **governança F3** (`:468-482`): com `compras_governanca_ativa` ON, o POST do cliente vira **ciência** — registra trilha e **não cria custo**; o custo sai pela cadeia interna de alçada (`compras.requisicao_emitir_pedido`) | Task 5 preserva as 3 camadas intactas — só a resolução do token muda. O escopo `APROVAR_COMPRA` passa a ter semântica dupla documentada (ciência sob governança / lançamento sem). O passo 5e-like de reescrever a query da compra seria **regressão** (mesma lição da adaptação nº 1 da F3) |
| R7 | Aprovação do portal atribuída ao admin (`usuario_id=compra.admin_id`), "só a identidade do portal resolve" | **CONFIRMADA** (e refinada) | `portal_obras_views.py:489-492` (comentário cita a Fase 9a nominalmente); `ESTADO-ATUAL.md:173`; docstring de `PortalAcessoEvento` (`models.py:5358-5368`) | Nenhum. Mas registrar: com governança ON o problema **encolhe** (o custo nasce autenticado na cadeia interna); a autoria errada persiste no fluxo com flag OFF e na ciência em si. A 9a continua sendo quem resolve a identidade |
| R8 | 7 rotas POST mutam estado só com a URL; sem login, sem CSRF, sem rate-limit, sem escopo (S1/S4/S6) | **CONFIRMADA** | POSTs: `portal_obras_views.py:425, 508, 539, 622, 749`; `propostas_consolidated.py:2507, 2591`. CSRF exempt um a um em `main.py:223-229` (+ `:236` para o PDF) e blueprint `propostas` inteiro em `app.py:1038-1049`. `Limiter` com `default_limits=[]`/`memory://` (`app.py:230-234`); único `@limiter.limit` do repo: `views/auth.py:14` | Só line numbers (eram :343/:377/:388/:432/:546 e :2503/:2587) |
| R9 | O token vaza nos logs; `_PATHS_SENSIVEIS` não cobre `/portal` (S7) | **CONFIRMADA** | `utils/auditoria_acesso.py:29` (`_PATHS_SENSIVEIS = ('/login', '/alterar-senha', '/usuarios')`), `:66-83` (loga `path=` inteiro; `[ESCRITA-ANONIMA]` em WARNING); gunicorn `--access-logfile -` com request line (`Dockerfile:97-101`) | Nenhum — Task 6 segue como escrita. Nota: a trilha da F3 (R3) grava IP/UA **em banco** mas não o token; o vazamento é só nos logs |
| R10 | `portal_pdf_extrato` lê o token da **querystring** (S7) | **CONFIRMADA** + **NOVA** | `medicao_views.py:512-521` idêntico ao descrito. **NOVA:** a rota **não checa `token_cliente_expira_em`** — token expirado pela F3 continua baixando o PDF do extrato (a F3 só blindou os dois helpers de `portal_obras_views.py`) | Task 5/6 (mover token para o path + resolver único) fecha o furo de tabela. Registrar no fecho da 9a que era um bypass da expiração da F3 |
| R11 | `admin_id = 10` chumbado no portal da proposta (S8) | **CONFIRMADA** | `propostas_consolidated.py:2473` (era :2469); irmão `return 10` em `:421`; rotas do portal da proposta em `:2457`, `:2507`, `:2591` | Só line numbers |
| R12 | Zero `Referrer-Policy`/CSP/Talisman; CDN no `_base.html` | **CONFIRMADA** | grep sem ocorrência em `*.py` fora de `archive/`; `templates/portal/_base.html:7-8` (jsdelivr + cloudflare) | Nenhum |
| R13 | Token nasce em 4 lugares, todos `secrets.token_urlsafe(32)` | **CONFIRMADA** | `views/obras.py:397` (era :375), `views/obras.py:1016` (era :949), `event_manager.py:1103` (era :1053), `portal_obras_views.py:682` (toggle, era :481) | Só line numbers. O comentário do toggle hoje é honesto ("NÃO rotaciona… reabrir reaproveita a MESMA URL" — achado nº 4 do review F3), alinhado ao que a Task 7 substitui |
| R14 | `MedicaoObra` sem nenhum campo de assinatura; ciclo `PENDENTE→APROVADO→FATURADO`; PDF vivo sem snapshot | **CONFIRMADA** | `MedicaoObra` `models.py:6103-6130` (era 5482), `MedicaoObraItem` `:6169`, `MedicaoContrato` `:6189`; `fechar_medicao` `services/medicao_service.py:180-204` (emite `obra.medicao_publicada` `:199-200`); PDF reportlab `:389+`, lê `obra.valor_contrato` vivo `:420`; link do PDF com `?token=` no template `templates/portal/_portal_medicoes.html:29` | Só line numbers |
| R15 | Zero `Assinatura`/`Contrato`/`Aditivo`/Drive no código | **CONFIRMADA** | grep em `models.py`: nenhuma classe; nenhum import `googleapiclient`/`google.oauth2`; `Lead.pasta` `models.py:7390` (era 6760) segue `String(500)` manual | Nenhum. A Fase 5 não rodou — a Task 9 cria a tabela genérica (o ramo preferível do plano) |
| R16 | Infra de eventos/webhook pronta para reuso (9b) | **CONFIRMADA** | `EventManager` `event_manager.py:13+`; allowlist `utils/webhook_dispatcher.py:59-75` — **ganhou 2 eventos da F2** (`obra.estado_alterado`, `obra.handoff`): são **10**, não 8; `WebhookEntrega` `models.py:7530`; painel `views/admin.py:263`; `_payload_obra_basico` ainda embute `portal_obra_url` com token (`utils/catalogo_eventos.py:86-107`, `:212`) — o risco RG segue real | Task 20: recontar a tabela §10 (10 atuais + 5 novos = **15** eventos, não 12) |
| R17 | Dependência da Fase 1 (papéis) — "teste de papel `xfail` até a Fase 1 entrar" | **QUEBRADA** (obsoleta, no bom sentido) | Fase 1 entregue: `utils/autorizacao.py`, `utils/identidade.py`, `UsuarioObra`, `PapelObra` existem; `PapelObra` ganhou `COMPRADOR` na F3 (`models.py:59-76`, migration 245) | Remover o fallback `xfail` da Task 5/7 — o teste de papel roda de verdade. **Ver R21 (NOVA)** sobre a flag |
| R18 | Dependência da Fase 2 (estados) — "Task 11 checa `Obra.status` textual como fallback e deixa `TODO-FASE-2`" | **QUEBRADA** (obsoleta) | Fase 2 entregue: `Obra.estado` (`models.py:364-368`, VARCHAR+CHECK, migration 231), `ObraTransicaoEstado` (`models.py:485`) | Task 11 usa `Obra.estado` direto (emitir token de assinatura exige obra em execução); apagar o fallback textual e o TODO. P2 de 9b: **satisfeita** |
| R19 | Dependência da Fase 6 (P1 — aditivo/versão de orçamento) | **CONFIRMADA** (segue em aberto) | grep `class OrcamentoVersao` em `models.py`: nada. Fase 6 não rodou (Fase 4 é a próxima) | Como o plano já prevê: Task 18 pulável, `orcamento_versao_id` nullable |
| R20 | Faixa de migrations **300-307** livre; registro em `migrations_to_run` | **CONFIRMADA** | Maior migration registrada: **247** (`migrations.py:4770`); nenhuma `migration_30x` no arquivo; F3 usou 240-247, F0.6 217-219, F1 214-216, F1.5 220-221, F2 230-232 | O plano diz "300–**309**" (linha 11 e mapa final), o `ESTADO-ATUAL.md:342` reserva "300-**307**". Sem colisão hoje (308-309 são "reservadas" e vazias), mas alinhar a redação no fecho. Atenção: a **Fase 4 está em andamento** num worktree (`.claude/worktrees/fase-4/`) e usará 250-254 — sem conflito |
| R21 | Emissão/revogação de acesso exige `PapelObra.GESTOR` na obra (Task 7) | **NOVA** | Achado nº 1 do review da F3: com `escopo_obra_ativo` **OFF**, `papel_na_obra` devolve GESTOR a **todo autenticado** do tenant (`utils/autorizacao.py:102-135`; `ESTADO-ATUAL.md:284`) — a exigência de papel colapsa em não-restrição | Task 7 precisa da mesma dependência dura da F3: em tenant com escopo OFF, o piso é `@admin_required` (o fallback que o plano já dava para "Fase 1 ausente" vira fallback para "flag desligada"); documentar no runbook da 9a, como o passo 0 do `docs/fase-3-rollout.md` |
| R22 | Lista de "quem lê `token_cliente`" da Task 8 (teste `test_nenhum_codigo_vivo_le_token_cliente`) | **NOVA** | Leitores novos desde 21/07: `templates/portal/_portal_compras.html:26,31,82,98` (o `:98` é o link do novo `ver_comprovante`), `scripts/smoke_test_demo_completo.py:119-125`, `scripts/seed_demo_alfa.py`, `scripts/seed_fixture_213_playwright.py`, `templates/obras/detalhes_obra_profissional.html:810-1516` (painel do link — o plano já mexe nele). E o grep na raiz pegaria **`.claude/worktrees/`** (worktree da F4 com cópia do código antigo) | Task 5/8: incluir `_portal_compras.html` na lista de templates a migrar (o plano lista só `_portal_medicoes/_portal_mapas_v2/_portal_rdos`); acrescentar `scripts/seed_`/`smoke_test_demo` aos `permitidos` (ou migrá-los) e **excluir `.claude/` do grep** do teste |
| R23 | Backfill (Task 4) dá aos legados os escopos D5 e "90 dias contados da migração" | **NOVA** (interação R1+R2) | Tokens carimbados pela F3 entre 23/07 e o deploy da 9a terão `token_cliente_expira_em` até ~jan/2027 | Regra do backfill: `expira_em = COALESCE(obra.token_cliente_expira_em, agora + default_do_tenant)`. Nunca encurtar validade vigente; nunca conceder `ASSINAR_MEDICAO` (inalterado) |

### Impacto no plano

**A Fase 9a continua executável e continua necessária — a Fase 3 entregou o paliativo, não a solução.** O que a F3 fez no portal (expiração 180d carimbada no toggle, trilha `portal_acesso_evento` com IP/UA, ciência-sem-custo sob governança) cobre S2 e S5 **parcialmente**: o token segue **em claro no banco**, sem hash, sem escopo, sem revogação por destinatário, sem uso único, vazando nos logs, e a autoria da ação do portal segue atribuída ao `admin_id` (R7). A arquitetura do plano (tabela `portal_acesso` + resolver único + `Assinatura`) permanece a resposta correta.

O que muda na execução:

1. **Task 3 muda de "criar" para "estender"**: `portal_acesso_evento` já existe (migration 247). Criar `portal_evento` paralela duplicaria trilha; o caminho é ALTER (acesso_id/escopo/resultado) — exatamente o tratamento condicional que a Task 9 já previa para `rdo_assinatura`.
2. **Tasks 1/4/8 absorvem `token_cliente_expira_em`**: o backfill herda a data carimbada pela F3 (nunca encurta), o default de validade é **180 dias** (decisão já tomada, R2), e a aposentadoria (305) zera também a coluna nova e remove `PRAZO_TOKEN_DIAS`/carimbo do toggle.
3. **Task 5 cobre 9 rotas em `portal_obras_views.py`** (nova `ver_comprovante`), preserva as camadas F0.6/F3 do corpo de `aprovar_compra`/`recusar_compra` (R6) e **atualiza em vez de quebrar** os testes da F3 — resolvendo explicitamente o conflito 404 (F3) × 410 (plano) para token expirado.
4. **Task 7 ganha a dependência de `escopo_obra_ativo`** (R21), espelhando o passo 0 do rollout da F3.
5. **Dependências simplificaram**: Fase 1 e Fase 2 entregues — caem o `xfail` e o `TODO-FASE-2`; Fase 6 segue ausente e a Task 18 segue pulável como o plano já previa. 9b: P2 satisfeita; P1, P3-P10 seguem abertas.
