...
| **E04** — `AlocacaoEquipe` + FK `rdo_gerado_id` | Conferência em base de **produção** antes do DROP; dev mediu 33 linhas com `rdo_gerado_id` vazio e ninguém provou o mesmo em produção. ⚠️ **Registro corrigido em 06/08 (B5.4):** os pontos vivos são **DOIS** — `views/rdo.py:536-538` e `services/importacao_fisico_financeiro.py:372-373`. O terceiro que esta linha listava (`crud_rdo_completo.py:557`, ex-`:539`) mora em `rdo_crud.excluir_rdo`, rota **sombreada** que nunca recebe despacho (🔬 url_map congelado em `tests/test_b5_rdo_crud_url_map.py`) — não protege base nenhuma | Contagem em produção, junto do gate de E02 (consulta [4] do `CONSULTAS-PRODUCAO-2026-08-06.md`) |
