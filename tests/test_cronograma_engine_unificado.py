...
**O caminho D saiu daqui em 05/08 (A19, Task B2.8), e a razão importa: o
número que ele caracterizava foi DESCARTADO, não migrado.**
`views/obras.calcular_progresso_real_servico` era a sétima variante da família
V1 e a única que respondia progresso de um SERVIÇO, não da obra. Os dois laços
que a chamavam gravavam em `servico['progresso']`, chave que nenhum template lê.
A função inteira foi removida — não há para onde apontar este teste, e
apontá-lo para `progresso_v1_acumulado` seria caracterizar outra pergunta.
