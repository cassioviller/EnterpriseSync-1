...
def _rdo(data):
    r = RDO(numero_rdo=f'RDO-F2-{_sfx()}', obra_id=_fx.obra.id, data_relatorio=data,
        # PORTE 24/08 — `estado` é EXPLÍCITO de propósito. Em 15/06 a coluna
        # não existia; hoje ela nasce 'rascunho' (models.py) e um RDO sem
        # estado declarado sairia da conta pelo filtro novo do read-model,
        # deixando o teste vermelho pelo motivo errado. 'preenchido' é o
        # estado do dia submetido (ESTADO_LEGADO do backfill da migration 260).
            admin_id=_fx.admin.id, status='Finalizado', criado_por_id=_fx.admin.id, estado='preenchido')
