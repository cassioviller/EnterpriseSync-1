...
def test_baia_importada_resultado_apos_apontamento():
    ...
    with app.app_context():
        ...
        r = RDO(numero_rdo=f'RDO-B-{_sfx()}', obra_id=obra_id, data_relatorio=date(2026, 6, 1),
                # PORTE 24/08 — `estado` explícito. RDO sem estado nasce
                # 'rascunho' (Fase 5) e o read-model o descarta: sem esta
                # linha o teste fica vermelho pelo motivo errado, medindo a
                # dívida de fixture em vez do que se propõe a medir.
...
@pytest.mark.skipif(not os.path.exists(XLSX), reason='arquivo de importação ausente')
def test_reimportar_no_mesmo_tenant_nao_duplica():
    """A idempotência que o Step 4 da Task 9 manda cobrir e que o E2E da
    branch afirmava mas não exercitava.

    Importar duas vezes no MESMO tenant não pode criar a segunda obra, o
    segundo orçamento nem a 29ª atividade. Contar é o que pega o defeito: a
    segunda chamada devolve `criado=False` e os mesmos ids, e as contagens no
    banco ficam onde estavam. Sem este teste, uma regressão na guarda de
    idempotência (`Orcamento` por numero + `Proposta.obra_id`) só apareceria
    em produção, no dia em que alguém apertasse o botão duas vezes.
    """
    with app.app_context():
        s = _sfx()
        admin = Usuario(username=f'baia_idem_{s}', email=f'baia_idem_{s}@sige.test',
                        nome='Baia Idem', tipo_usuario=TipoUsuario.ADMIN,
                        password_hash=generate_password_hash('Test@1234'),
                        versao_sistema='v2', ativo=True)
        db.session.add(admin); db.session.commit()

        from scripts.importar_baia_easypanel import importar_baia_completa
        primeiro = importar_baia_completa(admin.id, xlsx_path=XLSX)

        obras = Obra.query.filter_by(admin_id=admin.id).count()
        tarefas = TarefaCronograma.query.filter_by(
            obra_id=primeiro['obra_id']).count()
        imcs = ItemMedicaoComercial.query.filter_by(
            obra_id=primeiro['obra_id']).count()

        segundo = importar_baia_completa(admin.id, xlsx_path=XLSX)

        assert segundo['criado'] is False, (
            're-import no mesmo tenant se declarou criação nova')
        assert segundo['obra_id'] == primeiro['obra_id']
        assert segundo['orcamento_id'] == primeiro['orcamento_id']
        assert Obra.query.filter_by(admin_id=admin.id).count() == obras
        assert TarefaCronograma.query.filter_by(
            obra_id=primeiro['obra_id']).count() == tarefas
        assert ItemMedicaoComercial.query.filter_by(
            obra_id=primeiro['obra_id']).count() == imcs
