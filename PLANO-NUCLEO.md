...
> Documento vivo. Primeira versão: **2026-07-31**.
>
...
| 9 | Flags do núcleo prontas e desligadas | ✅ confirmado | Rollout puro — mas **não** zero-esforço: é trabalho operacional por tenant |
...
`rdo_percentual_livre` em tenant piloto, seguindo os runbooks.

- Confirmar deploy e migrations em produção
- **mpp:** pré-checagem por `scripts/diagnostico_cronograma_tenant.py` (a flag
  ...
  pós-import
- **editor v2:** snapshot antes, `--criar-baseline` nas obras datadas, decidir o
  calendário (o motor é seg–sex fixo nesta fase), ligar e comparar
...
**Pronto quando:** as três flags TRUE no tenant piloto, uma semana de operação
sem regressão de físico nem reclamação de datas, evidências nos runbooks.

**Decisão pendente:** tenant piloto e calendário. Se o piloto exigir sábado ou
domingo, a fase de calendário configurável sobe de prioridade — é o único
gatilho que transforma este pacote em código.
...
(fonte estável do BAC), p2 (flag ligada e validada no piloto).
...
| # | Decisão | Trava |
|---|---|---|
| 1 | **Tenant piloto do rollout + calendário** | A onda B inteira. O motor do editor v2 é seg–sex fixo nesta fase |
| 2 | **Dono do `valor_contrato`**: Fase 6 (cadeia) × Fase 9b (deltas) | p9, e por consequência o BAC do p10 |
| 3 | **Custo orçado: consertar no consumo ou na origem** | p3 |
