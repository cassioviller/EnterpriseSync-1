...
# PORTE 24/08 — este arquivo foi escrito em 15/06, antes de `RDO.estado`
# existir, e por isso não filtrava RDO de forma nenhuma: somava `RDOCustoDiario`
# e `RDOMaoObra` de todo RDO que encontrasse. `RDO.status` não serve de filtro —
# nasce 'Finalizado' para todo mundo (models.py, classe RDO). O ciclo de vida
# real mora em `RDO.estado` (services/rdo_ciclo_vida.ESTADOS: rascunho,
# preenchido, assinado, aprovado, retificado). Rascunho é documento que o autor
# ainda não submeteu; contá-lo como custo real faria o alarme (D5), o CPI e o
# EAC reagirem a ele — o mesmo defeito que 95eb585f fechou do outro lado.
...
def _horas_atividade_no_dia(tarefa_id, data):
    ...
    return _D(
        db.session.query(db.func.coalesce(db.func.sum(RDOMaoObra.horas_trabalhadas), 0))
        .join(RDO, RDO.id == RDOMaoObra.rdo_id)
        .filter(RDOMaoObra.tarefa_cronograma_id == tarefa_id,
                RDO.data_relatorio == data,
                RDO.estado != ESTADO_RASCUNHO)
        .scalar()
    )
