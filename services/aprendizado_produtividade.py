...
def produtividade_observada(subatividade_mestre_id, admin_id):
    ...
    SubatividadeMestre, a partir de RDOs JA SUBMETIDOS — isto e', qualquer
    estado que nao seja `rascunho`. Retorna (media, n_amostras)
    ...
    rows = (
        ...
        .filter(
            # PORTE 03/09 — era `RDO.status == 'Finalizado'`, escrito em
            # 15/06, antes de RDO.estado existir. `status` nasce
            # 'Finalizado' para todo RDO (models.py, classe RDO), entao
            # aquele filtro nao filtrava nada e o catalogo aprenderia com
            # rascunho. Estados validos em services/rdo_ciclo_vida.
            RDO.estado != 'rascunho',
