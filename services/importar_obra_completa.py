...
from app import db
from services.contrato_obra import (ORIGEM_IMPORTACAO_OBRA,
...
def _resolver_ou_criar_obra(proposta, orc, admin_id):
    ...
    obra = Obra(
        ...
        # PORTE 24/08 — `valor_contrato` NÃO é gravado aqui. Em 15/06 o campo
        # era escrita livre; a Fase 6 fez de `contrato_obra` o escritor único,
        # e é ele quem abre a `ObraContratoVersao` nº1. Gravar direto faria a
        # obra nascer com contrato e SEM baseline versionado — a trilha
        # `MedicaoContrato.contrato_versao_id` nasceria órfã, e a obra
        # importada seria a única do sistema sem versão de contrato.
        status='Em andamento',
    ...
    db.session.add(obra)
    db.session.flush()          # precisa de obra.id para abrir a versão nº1
    definir_valor_contrato(
        obra, float(orc.venda_total or 0),
        origem=ORIGEM_IMPORTACAO_OBRA,
        motivo=f'Importação de obra a partir do orçamento {orc.numero}')
    proposta.obra_id = obra.id
