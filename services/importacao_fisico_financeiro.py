...
# A implementação vive em `services/rdo_fotos_import.py` desde 27/07/2026 —
# `services/atualizacao_rdos.py` (caminho não-destrutivo) usa a mesma. O
# wrapper fica porque `FOTOS_RDO_BASE` precisa ser lido **na hora da
# chamada**: os testes trocam esse atributo de módulo por uma pasta tmp.
from services.rdo_fotos_import import (  # noqa: E402
    materializar_fotos_rdo as _materializar_fotos_rdo_base,
)


def _materializar_fotos_rdo(rdo, admin_id, dia, fotos):
    return _materializar_fotos_rdo_base(
        rdo, admin_id, dia, fotos, base=FOTOS_RDO_BASE, prefixo_log='FF_IMPORT')
