...
class RelatorioObraDigital(db.Model):
    ...
    # Dados básicos
    data_relatorio = db.Column(db.Date, nullable=False)
    numero_relatorio = db.Column(db.String(50), unique=True, nullable=False)
    
    # Campos Digital Mastery
    observacoes_gerais = db.Column(db.Text)
    condicoes_climaticas = db.Column(db.String(100))
    local_obra = db.Column(db.String(255), default='Campo')
    ...
    # Relacionamentos
    obra = db.relationship('Obra', backref='relatorios_digitais')
    admin = db.relationship('Usuario', foreign_keys=[admin_id], backref='relatorios_admin_digital')
    criado_por = db.relationship('Usuario', foreign_keys=[criado_por_id], backref='relatorios_criados_digital')
    
    def gerar_numero_relatorio(self):
        """Gera número único para Relatório Digital"""
        if not self.numero_relatorio:
            ano = self.data_relatorio.year
            count = db.session.query(func.count(RelatorioObraDigital.id)).filter(
                func.extract('year', RelatorioObraDigital.data_relatorio) == ano,
                RelatorioObraDigital.obra_id == self.obra_id
            ).scalar() or 0
            self.numero_relatorio = f"RD-{ano}-{count + 1:03d}"
    
    def __repr__(self):
        return f'<RelatorioObraDigital {self.numero_relatorio} - v10.0>'
