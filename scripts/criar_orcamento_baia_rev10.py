...
PORTE 24/08 (Task 9 do resgate) — MERGE consciente, não arquivo novo: este
script já existia na `main` com o mesmo corpo dentro de um `main()` que
pegava o primeiro ADMIN do banco. A branch extraiu esse corpo em
`criar_orcamento_baia(admin_id, xlsx_path)` — a forma reusável, que o E2E e o
importador por tela chamam — e deixou `main()` como invólucro de linha de
comando. O comportamento do CLI é o mesmo; o que muda é que agora existe uma
função que recebe o tenant em vez de adivinhá-lo. Os `open()` também deixaram
de vazar handle (`with`), e os imports de `Insumo`/`ComposicaoServico`, que a
branch trouxe sem usar, ficaram de fora.
"""
from decimal import Decimal
from app import app, db
from models import Usuario, Servico, Orcamento, OrcamentoItem
