...
| ~~28~~ **29** rotas `EXPOE DADO` sem auth | ✅ **triagem feita em 23/07** — `docs/anexos/B-triagem-rotas-expoe-dado.md` (📖 rota a rota). Eram 29, não 28. **9 já fechadas** pelas Fases 1/0.5 (o censo envelheceu), 5 públicas legítimas, 9 mortas (candidatas a remoção), 2 por token, 1 decisão do dono — e **3 a FECHAR**, sendo a pior `/funcionario_perfil/<id>/pdf` (dossiê de ponto de qualquer funcionário de qualquer tenant, anônimo). Fechar as 3 é trabalho pequeno e ainda não foi feito |
