...
| Item | Situação |
|---|---|
| ~~Triagem de `fix/bloco2-segredos` e `fix/bloco1-blindagem-acesso`~~ | ✅ **FECHADO POR INEXISTÊNCIA — 23/08.** 🔬 `git ls-remote --heads origin` (leitura anônima, sem credencial): **20 branches**, e nenhuma das duas está lá. A triagem não esperava `git fetch` autenticado — esperava branches que não existem mais. O diagnóstico velho fica abaixo como registro: ~~os branches nem existem localmente — a triagem exige `git fetch`. 🔬 27/07: o `git push` destravou, mas o `gh` segue deslogado; conferir se os branches ainda existem no remoto antes de planejar a triagem~~ |
