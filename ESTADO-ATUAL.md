...
| ~~28~~ **29** rotas `EXPOE DADO` sem auth | ✅ **triagem feita E executada em 23/07** — `docs/anexos/B-triagem-rotas-expoe-dado.md` (📖 rota a rota). Eram 29, não 28. **9 já fechadas** pelas Fases 1/0.5 (o censo envelheceu), 5 públicas legítimas, **3 fechadas** (`41034faa` — a pior era `/funcionario_perfil/<id>/pdf`, dossiê de ponto de qualquer tenant para anônimo) e **7 mortas removidas** (incl. as 4 APIs de RDO com cross-tenant latente e o template `novo_backup.html`). Restam: 2 por TOKEN (`/persistent-uploads/<path>` é a maior — escopar pelo token do portal) e 1 DECISÃO (`/health/veiculos`: fechar se nenhum monitor externo a consome) |
