...
**Princípio central:**
> Compra → GestaoCustoPai (MATERIAL) criado automaticamente → Estoque atualizado.
> Saída → Controle físico para rastreamento por obra/funcionário. Nenhum custo duplicado.
...
- O sistema cria automaticamente o **lançamento na Gestão de Custos** (categoria: Material de Obra)
...
2. Informe a **Obra** (opcional, mas recomendado para rastreabilidade)
3. Informe a **Data da Compra** e **Condição de Pagamento**
4. Adicione os **Itens da Compra**:
   - Para cada item, use o campo "Vínculo Almoxarifado" para ligar ao catálogo
   - Itens vinculados entram automaticamente no estoque ao salvar
5. Clique em **Salvar Compra**

### O que acontece automaticamente:

```
Compra Salva
├── GestaoCustoPai (MATERIAL) criado → aparece em Gestão de Custos
├── Se pagamento À Vista → status PAGO
├── Se prazo/boleto → status PENDENTE (entra no fluxo de aprovação)
└── Para cada item do catálogo:
    ├── AlmoxarifadoMovimento (ENTRADA) criado
    └── AlmoxarifadoEstoque (lote FIFO) atualizado
```

### Verificar Recebimento

Na tela de **Detalhe da Compra** (`/compras/<id>`), você vê:

| Coluna | Significado |
|--------|-------------|
| ✅ Recebido | Item já está no estoque |
| ⏳ Pendente | Item ainda não deu entrada (use botão "Registrar Recebimento") |
| ➖ Sem vínculo | Item não tem catálogo vinculado (controle manual externo) |

**Botão "Registrar Recebimento"** — aparece apenas quando há itens vinculados ainda pendentes. Cada item é processado uma única vez pelo total acumulado.
...
A saída registra **quem retirou**, **qual material** e **para qual obra**. Nenhum custo novo é gerado — o custo já foi reconhecido na compra.

### Passo a Passo — Saída

1. Selecione o **Funcionário** responsável pelo recebimento
2. Selecione a **Obra** destino (opcional, recomendado)
...
Aqui ficam todos os lançamentos financeiros, incluindo as compras de material.

### Filtros disponíveis

| Filtro | Como usar |
|--------|-----------|
| **Busca** | Digite o nome do fornecedor ou entidade |
| **Status** | Pendente, Solicitado, Pago, Parcial |
| **Categoria** | Selecione "Material de Obra" para ver só compras |
| **Obra** | Filtra lançamentos vinculados a uma obra específica |

### Como ver os lançamentos de material de uma obra

1. Acesse **Gestão de Custos**
2. No dropdown **"Todas as obras"**, selecione a obra desejada
3. No dropdown **"Todas as categorias"**, selecione **"Material de Obra"**
4. Clique em **Filtrar**

Cada linha mostra:
- **Categoria**: Material de Obra (ícone laranja)
- **Fornecedor**: Nome do fornecedor da compra
- **Obra**: Nome da obra (aparece em cinza abaixo do fornecedor)
- **Valor Total**: Total da nota fiscal
- **Status**: Pago / Pendente / etc.

Clique no **▶ (expandir)** para ver os detalhes de cada lançamento (itens, data, origem).
...
1. **Solicitar**: clique no ✈ e informe o valor a solicitar
2. **Aprovar**: clique no ✓, informe a data e banco
...
1. CADASTRAR itens no catálogo (uma vez por item)
      ↓
2. COMPRAR materiais (gera custo + entrada no estoque)
      ↓
3. EMITIR materiais para funcionário/obra (saída física)
      ↓
4. ACOMPANHAR custos em Gestão de Custos (por obra, por categoria)
      ↓
...
R: Não. O custo é reconhecido apenas na compra. A saída é só rastreamento físico.

**P: Posso comprar sem vincular ao catálogo?**
R: Sim. Itens sem vínculo ficam como "Sem vínculo" na tela de detalhe da compra. O custo é registrado normalmente, mas o estoque não é controlado automaticamente.
