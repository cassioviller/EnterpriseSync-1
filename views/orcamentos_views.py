...
def importar_obra(id):
    ...
    PORTE 24/08 (Task 6 do resgate da espinha) — a rota vive aqui, junto do
    orçamento que ela consome, e não no blueprint `resultado`: quem a aciona
    está na tela de edição do orçamento, e o `first_or_404` por
    (id, admin_id) é o mesmo escudo de tenant das vizinhas deste arquivo.
    """
    admin_id = _admin_id()
    orc = Orcamento.query.filter_by(id=id, admin_id=admin_id).first_or_404()
    try:
