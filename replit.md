...
## Recent Changes
- **2025-10-17:** PRODUCTION DEPLOYMENT FIX - Chart.js Local + 6 Dashboard Corrections - Resolvidos problemas críticos de gráficos não carregando em produção e cálculos inconsistentes. (1) Chart.js v4.4.0 hospedado localmente em /static/js/vendor/ (201KB) - CDN cdn.jsdelivr.net bloqueado em produção, 6 templates atualizados (base.html, base_completo.html, custos/dashboard.html, debug/mastery_dashboard.html, propostas/dashboard.html, relatorios.html). (2) Período de Diagnóstico Dinâmico - corrigidas 3 queries hardcoded (linhas 776, 791, 803) para usar variáveis data_inicio/data_fim, logs adicionados. (3) Filtro de Obras Ativas - linha 534 agora filtra ativo=True. (4) Variáveis de Template Completas - adicionadas custo_mao_obra, custo_alimentacao, custo_transporte, custo_outros, total_horas ao render_template. (5) Import ServicoObraReal Corrigido - utils.helpers → multitenant_helper. Architect-reviewed: PASS - segurança multi-tenant preservada, KPIs validados (Funcionários=48, Obras=40, Alimentação=R$1.107,36, Propostas Taxa=50%), teste e2e completo com screenshot, pronto para produção.
- **2025-10-17:** DASHBOARD COMPLETO - 7 Correções Críticas + Fallback Multi-Período - Resolvidos 14 problemas identificados em diagnóstico completo do dashboard. Correções implementadas: (1) Período Dinâmico Multi-Tenant Seguro - query MAX(data) agora executa APÓS admin_id estar definido com JOIN filtrado `FROM registro_ponto rp JOIN funcionario f ON rp.funcionario_id = f.id WHERE f.admin_id = :admin_id`, elimina vazamento cross-tenant. (2) Fallback de Custos Multi-Período - corrigido cálculo de dias úteis para TODO o período (não apenas primeiro mês), multiplica salário por número de meses, suporta filtros personalizados de 1-12 meses. (3) Exception Handling Inteligente - preserva variáveis já calculadas, não reseta todos valores para 0 em erro. (4) Ordenação de Execução - cálculo de veículos movido ANTES de custos. (5) Departamentos Protegidos - conversão dict→list com verificação de tipo. (6) Filtro de Período em Propostas COMPLETO - todas queries (aprovadas, enviadas, rascunho, rejeitadas, total_propostas) filtram por data_proposta. (7) Valor Médio de Propostas - filtra valor_total > 0, NOT NULL + período. Architect-reviewed: PASS em todas correções - isolamento multi-tenant completo, métricas consistentes, pronto para produção.
