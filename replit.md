...
-   **Core Modules:**
    ...
    -   **Reembolsos V2 (Migration 79/80/81 + 2026-04-02 CRUD):** `reembolso_views.py` blueprint at `/reembolsos`; full CRUD (listar, novo, editar, excluir); uses `registrar_custo_automatico()` on create to integrate with Gestão de Custos V2; stores `gestao_custo_pai_id` FK on reembolso; status filter applied via `GestaoCustoPai.status` lookup; templates at `templates/reembolsos/`; V2-only navbar link under "Gestão Financeira" dropdown.
    ...
    -   **Gantt Drag & Drop Interativo (2026-04-02):** `templates/obras/cronograma.html` upgraded with two drag interactions: (1) Horizontal Gantt bar drag — mousedown/mousemove/mouseup on each bar calls PATCH `/tarefa/<id>` with new `data_inicio`, triggering chain recalc; (2) Row reorder via HTML5 drag API — grip handle (`fa-grip-vertical`) on each table row triggers `dragstart/dragend` posting new order to `/reordenar`. Both interactions provide toast feedback on success/error.
