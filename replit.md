...
-   **Core Modules:**
    ...
    -   **Construction Project Management (RDO):** Daily Work Reports and dynamic service progress. **Route Consolidation (v9.0):** Sistema utiliza blueprint único `rdo_editar_bp` (rdo_editar_sistema.py) para edição de RDOs, eliminando conflitos de rotas. Rotas legadas em `crud_rdo_completo.py` e `views.py` redirecionam automaticamente para o blueprint oficial, garantindo consistência. Template: `rdo/editar_rdo.html`. Apenas RDOs em status 'Rascunho' podem ser editados, com validação multi-tenant via admin_id.
    ...
    -   **Accounting (Contabilidade):** Double-entry bookkeeping, automatic journal entries from payroll, a chart of accounts, trial balance, and DRE with competency selection. **Automated Payroll Integration (v9.0):** Fully automated accounting entry generation when payroll is processed via event-driven architecture. Event `folha_processada` triggers `criar_lancamento_folha_pagamento` handler that creates journal entries with proper multi-tenant isolation. Structure: DEBIT 5.1.01.001 (Salários) for total_proventos + fgts, CREDITS to 2.1.02.001 (Salários a Pagar) for net salary, 2.1.02.002 (INSS a Recolher), 2.1.03.001 (IRRF a Recolher), and 2.1.02.003 (FGTS a Recolher). Chart of accounts includes complete tax structure (2.1.03 IMPOSTOS E TAXAS) with all required accounts seeded automatically.
    ...
    -   **Shared Device Time Clock System (Ponto Eletrônico):** Mobile-first time clock with GPS tracking. **Sistema de importação via Excel** permite importar registros de ponto em lote com planilha modelo gerada automaticamente (uma aba por funcionário ativo), validação robusta de formatos de data/hora, tratamento de duplicatas (atualiza registros existentes), importação parcial com relatório detalhado de erros, e interface drag-and-drop moderna. **Seletor de competência** permite escolher mês/ano específico ao baixar o modelo (input type="month" mobile-friendly), com validação automática que impede seleção de meses futuros e nome de arquivo dinâmico refletindo a competência escolhida (ex: modelo_ponto_202410.xlsx). **Planilha Excel aprimorada v4.0** inclui: (1) Aba de Legenda/Dicionário com 11 tipos de registro (TRAB, FALTA, SAB_TRAB, DOM_TRAB, etc) e instruções detalhadas; (2) Seção "🏗️ OBRAS DISPONÍVEIS" na legenda listando todas as obras ativas (ID, Nome, Cliente) para referência; (3) **Exibição do horário padrão do funcionário** (entrada e saída) buscado de `HorarioTrabalho`; (4) Dropdown de validação de dados na coluna "Tipo" para seleção facilitada; (5) Coluna "Obra ID" com dropdown vinculado às obras ativas, permitindo associar registros de ponto a projetos específicos; (6) **Colunas calculadas automaticamente**: "Min. Atraso" (coluna H) calcula minutos de atraso quando entrada > horário padrão, e "Min. HE" (coluna I) calcula minutos de horas extras quando saída > horário padrão, usando fórmulas Excel com valores de tempo reais armazenados em células ocultas (Z1/Z2); (7) Seção de KPIs automáticos com 10 indicadores calculados por fórmulas Excel: dias trabalhados, faltas, faltas justificadas, horas extras 50%, horas extras 100%, férias, **total minutos de atraso**, **total minutos de horas extras**, **horas extras (HH:MM)**, e **atrasos (HH:MM)** que atualizam em tempo real; (8) Suporte a tipos especiais sem horários (faltas, folgas, férias, atestados); (9) Importação salva obra_id no banco (campo nullable) para rastreamento de trabalho por projeto.
