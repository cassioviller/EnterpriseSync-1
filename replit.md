...
-   **Core Modules:**
    ...
    -   **Gestão de Custos V2 (Migration 77):** Centralized cost management module with approval workflow. `GestaoCustoPai` + `GestaoCustoFilho` models (hierarchy); `gestao_custos_views.py` blueprint at `/gestao-custos`; status flow PENDENTE → SOLICITADO → PAGO; auto-creates `FluxoCaixa` on payment authorization; AJAX accordion to expand child items; manual cost entry form; `utils/financeiro_integration.py` with `registrar_custo_automatico()` for automatic integration from other modules; V2-only menu item "Gestão de Custos" in navbar.
