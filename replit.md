...
-   **Core Modules:**
    ...
    -   **Gestão de Custos V2:** Centralized cost management with a 4-step approval workflow and integration with other modules for automatic cost registration. Per-line CRUD: each `GestaoCustoFilho` row in the AJAX-expanded view has Edit/Delete buttons. `POST /gestao-custos/filho/<id>/editar` updates data/descricao/valor/obra and recomputes `pai.valor_total`; `POST /gestao-custos/filho/<id>/excluir` cascades to the source table (`registro_ponto*`, `rdo_mao_obra`, `lancamento_transporte`, `lancamento_alimentacao`, `reembolso_funcionario`), recomputes the pai or auto-deletes it (with linked `FluxoCaixa`) when the last filho is removed. Both blocked when pai status is PAGO/RECUSADO. Origens sem tabela-fonte (`importacao_diaria*`, `manual`, `diaria_manual`, `diaria_abril`, `conta_pagar`) são tratadas como no-op silencioso na cascata.
    -   **Reembolsos V2:** Full CRUD for reimbursements, integrated with Gestão de Custos V2.
    -   **Catálogo de Serviços + Orçamento Paramétrico (Task #82):** Catálogo cross-obra de Insumos (`Insumo`, tipo MATERIAL/MAO_OBRA/EQUIPAMENTO) com histórico versionado de preço (`PrecoBaseInsumo`, vigência por data). Cada `Servico` tem composição (`ComposicaoServico`: insumo × coeficiente) e preço de venda calculado por `services/orcamento_service.py` via `preco = custo / (1 − imposto_pct/100 − margem_lucro_pct/100)`. Defaults de imposto/lucro em `ConfiguracaoEmpresa.imposto_pct_padrao/lucro_pct_padrao`. Blueprint `views/catalogo_views.py` em `/catalogo/insumos` (CRUD), `/catalogo/servicos/<id>/composicao` (composição + recálculo) e `/catalogo/servicos/<id>/historico-obras` (performance cross-obra). APIs `/catalogo/api/servicos/buscar` e `/catalogo/api/insumos/buscar`. Quando uma proposta é aprovada, `handlers/propostas_handlers.py::_propagar_proposta_para_obra` cria automaticamente um `ItemMedicaoComercial` por `PropostaItem` na obra vinculada; o listener `after_insert` em `ItemMedicaoComercial` (models.py) cria o `ObraServicoCusto` pareado herdando `valor_comercial → valor_orcado` e `servico_id → servico_catalogo_id`, fechando o ciclo catálogo → proposta → medição comercial → planejamento de custo. Migration #121 adiciona `insumo`, `preco_base_insumo`, `composicao_servico`, `servico.imposto_pct/margem_lucro_pct/preco_venda_unitario`, `proposta_itens.servico_id`, `item_medicao_comercial.servico_id/quantidade`, `obra_servico_custo.servico_catalogo_id`, `configuracao_empresa.imposto_pct_padrao/lucro_pct_padrao`. Rotas auxiliares POST `/catalogo/proposta-itens/<id>/vincular-servico` e `/catalogo/medicao-itens/<id>/vincular-servico` permitem vincular itens legados (sem `servico_id`) ao catálogo a posteriori.
